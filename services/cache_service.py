...
from config.config import (
    ...
    CACHE_EXTENSION_DURATION, LOG_LEVEL
)
...
def _create_new_gemini_cache(inventory_data: str) -> str:
    ...
    try:
        ...
        if system_instruction_text is None:
            logger.warning("System prompt not found in Firestore. Using default instruction.")
            # Default instruction without placeholder
            system_instruction_text = (
                "You are a helpful customer support chatbot. Use the provided inventory data "
    ...
    system_part = Part.from_text(text=system_instruction_text)
    content_part = Part.from_text(text=inventory_data) # Assumes inventory_data is valid text
